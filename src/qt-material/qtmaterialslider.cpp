...
void QtMaterialSliderPrivate::init() {
    ...
    if (q->orientation() == Qt::Vertical) { sp.transpose(); }
...
QRectF QtMaterialSliderPrivate::trackBoundingRect() const {
    ...
    return Qt::Horizontal == q->orientation() ? QRectF(
...
QRectF QtMaterialSliderPrivate::thumbBoundingRect() const {
    ...
    return Qt::Horizontal == q->orientation() ? QRectF(
               thumb->offset(),
               q->height() / 2 - QT_MATERIAL_SLIDER_MARGIN,
               QT_MATERIAL_SLIDER_MARGIN * 2,
               QT_MATERIAL_SLIDER_MARGIN * 2)
                                              : QRectF(
                                                  q->width() / 2 - QT_MATERIAL_SLIDER_MARGIN,
                                                  thumb->offset(),
                                                  QT_MATERIAL_SLIDER_MARGIN * 2,
                                                  QT_MATERIAL_SLIDER_MARGIN * 2);
}
...
void QtMaterialSlider::sliderChange(SliderChange change) {
    ...
    if (SliderOrientationChange == change) {
        QSizePolicy sp(QSizePolicy::Expanding, QSizePolicy::Fixed);
        if (orientation() == Qt::Vertical) { sp.transpose(); }
        setSizePolicy(sp);
...
void QtMaterialSlider::updateThumbOffset() {
    ...
    const int offset = QtMaterialStyle::sliderPositionFromValue(
        ...
        maximum(),
        sliderPosition(),
        Qt::Horizontal == orientation() ? width() - QT_MATERIAL_SLIDER_MARGIN * 2
