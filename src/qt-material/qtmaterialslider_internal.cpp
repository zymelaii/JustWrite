...
QtMaterialSliderStateMachine::QtMaterialSliderStateMachine(
    QtMaterialSlider *slider, QtMaterialSliderThumb *thumb, QtMaterialSliderTrack *track)
...
bool QtMaterialSliderThumb::eventFilter(QObject *obj, QEvent *event) {
    if (QEvent::ParentChange == event->type()) { setParent(m_slider->parentWidget()); }
...
void QtMaterialSliderThumb::paintEvent(QPaintEvent *event) {
    ...
    QRectF halo(
        (m_slider->pos() - QPointF(m_haloSize, m_haloSize) / 2) + disp,
    ...
    QRectF geometry = Qt::Horizontal == m_slider->orientation()
                        ? QRectF(
                              m_offset,
                              m_slider->height() / 2 - QT_MATERIAL_SLIDER_MARGIN,
                              QT_MATERIAL_SLIDER_MARGIN * 2,
                              QT_MATERIAL_SLIDER_MARGIN * 2)
                              .translated(m_slider->pos())
                        : QRectF(
                              m_slider->width() / 2 - QT_MATERIAL_SLIDER_MARGIN,
                              m_offset,
                              QT_MATERIAL_SLIDER_MARGIN * 2,
                              QT_MATERIAL_SLIDER_MARGIN * 2)
                              .translated(m_slider->pos());
...
void QtMaterialSliderTrack::paintEvent(QPaintEvent *event) {
    ...
    if (Qt::Horizontal == m_slider->orientation()) {
        painter.translate(
            m_slider->x() + QT_MATERIAL_SLIDER_MARGIN,
            m_slider->y() + m_slider->height() / 2 - static_cast<qreal>(m_trackWidth) / 2);
    } else {
        painter.translate(
            m_slider->x() + m_slider->width() / 2 - static_cast<qreal>(m_trackWidth) / 2,
    ...
    if (m_slider->invertedAppearance()) { qSwap(bgRect, fgRect); }
