...
public:
    ...
    const DictUnit*
        ...
        for (RuneStrArray::const_iterator it = begin; it != end; it++) {
            if (NULL == ptNode->next) { return NULL; }
            citer = ptNode->next->find(it->rune);
            if (ptNode->next->end() == citer) { return NULL; }
            ptNode = citer->second;
        }
    ...
    void Find(
        ...
        for (size_t i = 0; i < size_t(end - begin); i++) {
            ...
            for (size_t j = i + 1; j < size_t(end - begin) && (j - i + 1) <= max_word_len; j++) {
                if (ptNode == NULL || ptNode->next == NULL) { break; }
                citer = ptNode->next->find((begin + j)->rune);
                if (ptNode->next->end() == citer) { break; }
                ptNode = citer->second;
    ...
    void InsertNode(const Unicode& key, const DictUnit* ptValue) {
        ...
        for (Unicode::const_iterator citer = key.begin(); citer != key.end(); ++citer) {
            if (NULL == ptNode->next) { ptNode->next = new TrieNode::NextMap; }
    ...
    void DeleteNode(const Unicode& key, const DictUnit* ptValue) {
        ...
        for (Unicode::const_iterator citer = key.begin(); citer != key.end(); ++citer) {
            // 链表不存在元素
            if (NULL == ptNode->next) { return; }
            kmIter = ptNode->next->find(*citer);
            // 如果map中不存在,跳出循环
            if (ptNode->next->end() == kmIter) { break; }
            // 从unordered_map中擦除该项
...
private:
    void CreateTrie(const vector<Unicode>& keys, const vector<const DictUnit*>& valuePointers) {
        if (valuePointers.empty() || keys.empty()) { return; }
        assert(keys.size() == valuePointers.size());

        for (size_t i = 0; i < keys.size(); i++) { InsertNode(keys[i], valuePointers[i]); }
    }
