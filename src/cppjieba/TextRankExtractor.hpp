    ...
    public:
        ...
        void rank(WordMap& ws, size_t rankTime = 10) {
            ...
            for (WordMap::iterator i = ws.begin(); i != ws.end(); i++) {
                if (i->second.weight < min_rank) { min_rank = i->second.weight; }
                if (i->second.weight > max_rank) { max_rank = i->second.weight; }
            }
            for (WordMap::iterator i = ws.begin(); i != ws.end(); i++) {
                ws[i->first].weight =
...
public:
    ...
    void Extract(const string& sentence, vector<string>& keywords, size_t topN) const {
        ...
        for (size_t i = 0; i < topWords.size(); i++) { keywords.push_back(topWords[i].word); }
    }

    void
        Extract(const string& sentence, vector<pair<string, double>>& keywords, size_t topN) const {
        vector<Word> topWords;
        Extract(sentence, topWords, topN);
...
private:
    void LoadStopWordDict(const string& filePath) {
        ...
        string line;
        while (getline(ifs, line)) { stopWords_.insert(line); }
